...
const ExpulsionButton = ({ idUser }) => {
  ...
  const isUserAdmin = async (serverId, userId) => {
    await axios.get("https://localhost:3000/serveur/", {
      params: {
        idServeur: contexteID,
      },
    });
    return userData.roles.some(
      (role) => role.serverId === serverId && role.isAdmin
    );
  };
  useEffect(() => {
  ...
  return (
    <div>
      {isAdmin && (
        <button onClick={handleExpulsion}>Bannir l'utilisateur {userId}</button>
      )}
    </div>
  );
};
...
export { DeleteChannelButton, CreateChannelButton, ExpulsionButton };
