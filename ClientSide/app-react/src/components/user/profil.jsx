...
const Profil = () => {
  ...
  useEffect(() => {
    ...
    const groupesFiltres = chatGroups.filter(
      (groupeId) => groupeId.MembreGroupe.length === 2
    );

    setGroupesDeuxUtilisateurs(groupesFiltres);
    [chatGroups];
  ...
  return (
    <PopUp>
      ...
      <div className="userinfo">
        ...
        <p>Bio : {User.Bio}</p>
        {estAmi ? (
          <button disabled>Déjà Ami</button>
        ) : (
          <button onClick={testAjouterAmi}>Ajouter en ami</button>
        )}
        <button onClick={() => TestEnvoyerMessage(groupeId)}>
          Envoyer un message
        </button>
      </div>
    </PopUp>
